...
import { ALL_OGX_OPPORTUNITIES } from '../../apollo/queries/opportunities';

const SwitchDepatments = () => {
  const location = useHistory();
  const goTo = (to) => location.push(to);
  useEffect(() => {
    ALL_OGX_OPPORTUNITIES().then((res) => {
      console.log(res);
    });
  }, []);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Box
            onClick={() => goTo('/ogx-opportunities')}
            p="2rem 4rem"
            style={{
              background: 'white',
              display: 'flex',
        ...
        >
          <Box
            onClick={() => goTo('/icx-opportunities')}
            p="2rem 4rem"
            style={{
              background: 'white',
              display: 'flex',
