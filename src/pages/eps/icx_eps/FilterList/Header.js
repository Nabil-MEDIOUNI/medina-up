...
import { checkICXEPFilters } from '../../../../utils/checkFilters';
...
const FilterHeader = ({
  ...
  return (
    <Box className={classes.filtersHeader}>
      <Box ml={1}>
        {(showClearBtn && checkICXEPFilters()) && (
      ...
      <Box mr={1}>
        <Button disabled={!checkICXEPFilters()} onClick={() => setFilter(false)}>
          <Typography style={{
            color: !checkICXEPFilters() ? '#ffffff80' : 'white',
            fontSize: 14,
            opacity: 1,
          }}
          >
